    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow rounded p-6">
                ...
                <form method="POST" action="{{ route('pdn.update', $pdn->id) }}">
                    {{-- Formulario de datos. --}}
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="descripcion" rows="3"
                                  class="w-full border rounded px-3 py-2">{{ old('descripcion', $pdn->descripcion) }}</textarea>
                    </div>

                    <div class="mb-4">
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="activo" value="1"
                                   {{ old('activo', $pdn->activo) ? 'checked' : '' }}>
                            <span>Activo</span>
                        </label>
                    </div>
